...
auth()
{
    ...
    stat_code=0
    stat_code=$(curl --interface macvlan2 -s -w %{http_code} --connect-timeout 5 "https://www.google.cn/generate_204")
    if [ "${stat_code}" -eq "204" ]; then echo "Internet connected."
        else echo "Failed to connect to the Internet. Please check the username and password and try again."
    fi

    stat_code=0
    stat_code=$(curl --interface macvlan3 -s -w %{http_code} --connect-timeout 5 "https://www.google.cn/generate_204")
    if [ "${stat_code}" -eq "204" ]; then echo "Internet connected."
        else echo "Failed to connect to the Internet. Please check the username and password and try again."
    fi
}
